...
type Item = string | { value: string; label: ReactNode; icon?: ReactNode; disabled?: boolean }
...
interface MV { x: MotionValue<number>; sx: MotionValue<number>; sy: MotionValue<number> }

type ChipOwnProps = { mv: MV; children: ReactNode }
type ChipButtonProps = Omit<
  React.ButtonHTMLAttributes<HTMLButtonElement>,
  'onDrag' | 'onDragEnd' | 'onDragEnter' | 'onDragExit' | 'onDragLeave' | 'onDragOver' | 'onDragStart'
>

const Chip = forwardRef<HTMLButtonElement, ChipOwnProps & ChipButtonProps>(
  function Chip({ mv, children, ...rest }, ref) {
    const transform = useTransform(() => `translateX(${mv.x.get()}px) scale(${mv.sx.get()}, ${mv.sy.get()})`)
    return <motion.button ref={ref} style={{ transform }} {...(rest as object)}>{children}</motion.button>
  }
)
...
export function JellyRadio({
  ...
  const mvFor = (i: number): MV => {
    if (!mvs.current[i]) {
      mvs.current[i] = { x: motionValue(0), sx: motionValue(1), sy: motionValue(1) }
    }
  ...
  const apply = (sel: number, instant?: boolean) => {
    ...
    for (let i = 0; i < C.count; i++) {
      ...
      if (instant || C.reduce) {
        mv.x.jump(x); mv.sx.jump(s); mv.sy.jump(s); continue
      }
      ...
      const j = C.jelly
      animate(mv.sx, s, { ...spring(k * (1 + 0.24 * j), 0.9 - 0.1 * j, Math.min(0.85, C.bounce + 0.3 * j)), delay })
  ...
  useLayoutEffect(() => {
    const settle = () => { measure(); apply(applied.current, true) }
    settle()
  ...
  useEffect(() => () => mvs.current.forEach((mv) => { mv?.x.destroy(); mv?.sx.destroy(); mv?.sy.destroy() }), [])
    ...
    >
      {list.map((it, i) => (
        <Chip
          ...
          onClick={(e) => commit(i, (e as any).detail === 0)}
